...
function extract_single_value<U, T extends ArrayLike<U>>(
  array: T,
  index: number
): ExtractResult<T> {
...
function extract_matrix_range<U, T extends ArrayLike<U>>(
  ...
  assert(
    dimension,
    'expecting dimension to know the shape of the matrix represented by the flat array'
  );
...
function insertInPlaceStandardArray(
  arrayToAlter: any[],
  low: number,
  high: number,
  newValues: any[]
...
function insertInPlaceTypedArray(arrayToAlter: any, low: number, high: number, newValues: any) {
