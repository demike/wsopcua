...
function readerHeader(data: DataView): PacketInfo {
  ...
  return {
    length: length,
    extra: msgType,
    messageHeader: { isFinal: 'F', length: length, msgType: msgType },
  };
}
...
describe('PacketAssembler', function () {
  it('should assemble a single packet', async function () {
    await new Promise<void>((resolve) => {
      const packet_assembler = new PacketAssembler({
        readMessageFunc: readerHeader,
  ...
  it('should assemble a message sent over several packets', async function () {
    await new Promise<void>((resolve) => {
      const packet_assembler = new PacketAssembler({
        readMessageFunc: readerHeader,
        minimumSizeInBytes: 5,
  ...
  it('should assemble a message sent one byte at a time', async function () {
    await new Promise<void>((resolve) => {
      const packet_assembler = new PacketAssembler({
        readMessageFunc: readerHeader,
        minimumSizeInBytes: 5,
      }).on('message', function (msg: DataView) {
        const info = readerHeader(msg);
        expect(info.length).toBe(msg.byteLength);
  ...
  it('should deal with packets containing data from 2 different messages', async function () {
    let counter = 0;
    await new Promise<void>((resolve) => {
      const packet_assembler = new PacketAssembler({
        readMessageFunc: readerHeader,
