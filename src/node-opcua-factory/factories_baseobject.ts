...
import {getFactory} from  "./factories_factories";
import  {_defaultTypeMap} from "./factories_builtin_types");
 ...
 * @return {Number}
 */
binaryStoreSize(options) : number {
...
private static _exploreObject = function (self, field, data, args) {
...
explore() : string {
    ...
    apply_on_all_schema_fields(this, this._schema, data, _exploreObject,arguments);
    data.lines.push("};");
    return data.lines.join("\n");
};

protected _visit_schema_chain(self, schema, options, func, extra_data) : void {
...
private static _JSONify(schema, options) : string {
...
private construct_param(schema, options) {
    /* jshint validthis: true */
    var self = this;
...
clone(/*options,optionalfilter,extraInfo*/) : BaseUAObject {
    var self = this;

    var params = {};

    this.construct_param(this._schema,params);
