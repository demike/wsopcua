...
export class TCP_transport extends EventEmitter<TcpTransportEvents> {
  ...
  protected _write_chunk(message_chunk: string | ArrayBufferLike | Blob | ArrayBufferView) {
    if (this._socket) {
      this.bytesWritten += (message_chunk as ArrayBufferLike).byteLength
        ? (message_chunk as ArrayBufferLike).byteLength
        : (message_chunk as string).length;
   ...
   * @protected
   */
  protected _install_socket(socket: any) {
    assert(socket);
