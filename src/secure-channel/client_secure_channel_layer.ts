...
export class ClientSecureChannelLayer extends EventEmitter<ClientSecureChannelLayerEvents> implements ITransactionStats {
    ...
    this.connectionStrategy.maxRetry =
            utils.isNullOrUndefined(options.connectionStrategy.maxRetry) ? 10 : options.connectionStrategy.maxRetry;
...
protected _install_security_token_watchdog() {
    ...
    if (doDebug) {
        debugLog(' time until next security token renewal = ' + timeout + '( lifefime = ' + lifeTime + ')');
    }
