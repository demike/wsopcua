...
export class ClientSubscription extends EventEmitter<ClientSubscriptionEvents> {
  ...
  constructor(session: ClientSession, options: ICreateSubscriptionRequest) {
    ...
    options.maxNotificationsPerPublish =
      options.maxNotificationsPerPublish === null ||
      options.maxNotificationsPerPublish === undefined
        ? 0
        : options.maxNotificationsPerPublish;
  ...
  protected __on_publish_response_DataChangeNotification(
    notification: subscription_service.DataChangeNotification
  ) {
    ...
    monitoredItems.forEach((monitoredItem) => {
      ...
      if (monitorItemObj) {
        if (monitorItemObj.itemToMonitor.attributeId === AttributeIds.EventNotifier) {
          console.log(
            'Warning: Server send a DataChangeNotification for an EventNotifier. EventNotificationList was expected'
          );
          console.log(
            '         the Server may not be fully OPCUA compliant. This notification will be ignored.'
          );
        } else {
  ...
  protected __on_publish_response_StatusChangeNotification(
    notification: subscription_service.StatusChangeNotification
  ) {
  ...
  protected __on_publish_response_EventNotificationList(
    notification: subscription_service.EventNotificationList
  ) {
      ...
      notificationData.forEach((notification) => {
        ...
        switch (notification.constructor) {
          case subscription_service.DataChangeNotification:
            ...
            this.__on_publish_response_DataChangeNotification(
              <subscription_service.DataChangeNotification>notification
            );
            break;
          case subscription_service.StatusChangeNotification:
            this.__on_publish_response_StatusChangeNotification(
              <subscription_service.StatusChangeNotification>notification
            );
            break;
          case subscription_service.EventNotificationList:
            this.__on_publish_response_EventNotificationList(
              <subscription_service.EventNotificationList>notification
            );
            break;
          default:
 ...
 */
  public monitor(
    ...
    requestedParameters: IMonitoringParameters,
  ...
  public monitorItems(
    ...
    requestedParameters: IMonitoringParameters | IMonitoringParameters[],
    timestampsToReturn?: TimestampsToReturn,
    done?: (err: Error | null, mItemGroup?: MonitoredItemGroup) => void
  ): MonitoredItemGroup {
  ...
  public _delete_monitored_items(
    monitoredItems: MonitoredItemBase[],
  ...
  protected _delete_monitored_item(
  ...
  public setPublishingModeP(publishingEnabled: boolean): Promise<void> {
    return new Promise((res, rej) => {
      this.setPublishingMode(publishingEnabled, (err) => {
        if (err) {
          rej(err);
        } else {
          res();
      ...
      [
        ...
        (cb: ErrorCallback) => {
          ...
          for (const key in monitoredItems_old) {
            ...
            itemsToCreate.push(
              new subscription_service.MonitoredItemCreateRequest({
            ...
            if (!err) {
              ...
              monitoredItemResults.forEach((monitoredItemResult, index) => {
                ...
                if (monitoredItemResult.statusCode === StatusCodes.Good) {
                  ...
                  debugLog(
                    'monitoredItemResult.statusCode = ' + monitoredItemResult.statusCode.toString()
                  );
                }
              });
            }
  ...
  public recreateSubscriptionAndMonitoredItemP(): Promise<void> {
    return new Promise((res, rej) => {
      this.recreateSubscriptionAndMonitoredItem((err) => {
        if (err) {
          rej(err);
        } else {
          res();
