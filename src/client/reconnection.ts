...
function repair_client_session_by_recreating_a_new_session(
  ...
  if (doDebug) {
    debugLog(
      ' repairing client session by_recreating a new session ',
  ...
  async_series(
    [
      ...
      function create_new_session(callback: ErrorCallback) {
        ...
        (<any>client).__createSession_step2(
          session,
          function (err: Error, _new_session: ClientSession) {
      ...
      function attempt_subscription_transfer(callback: ErrorCallback) {
        ...
        debugLog(
          ...
          new_session.getPublishEngine().subscriptionCount
        );
        ...
        debugLog(
          '    => asking server to transfer subscriptions = [',
          subscriptionsIds.join(', '),
          ']'
        );
        ...
        new_session.transferSubscriptions(
          new TransferSubscriptionsRequest(subscriptionsToTransfer),
          (err, transferSubscriptionsResponse) => {
            ...
            for (let i = 0; i < results.length; i++) {
              ...
              if (statusCode === StatusCodes.BadSubscriptionIdInvalid) {
                // repair subscription
                debugLog(
                  '         WARNING SUBSCRIPTION  ' + subscriptionsIds[i] + ' SHOULD BE RECREATED'
                );
            ...
            debugLog(
              '  new session subscriptionCount = ',
              new_session.getPublishEngine().subscriptionCount
            );

            async_map(
              subscriptions_to_recreate,
              function recreate_subscription(subscriptionId: number, next: () => void) {
                ...
                subscription.recreateSubscriptionAndMonitoredItem(function (error?: Error) {
                  ...
                  debugLog(
                    '          => RECREATING SUBSCRIPTION  AND MONITORED ITEM DONE ',
                    subscriptionId
                  );
                  next();
                });
