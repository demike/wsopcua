...
function makeSubscription(session: ClientSession, subscriptionId: number, timeoutHint: number): ClientSubscription {
    /*
...
describe('Testing the client publish engine', function () {
    ...
    beforeEach(function () {
        ...
        spyOn(<any>window, 'setImmediate').and.callFake((callback) => {
    ...
    it('a client should send a publish request to the server for every new subscription', function () {
        ...
        publish_spy.and.callFake(function() { publish_args = arguments; });
    ...
    function () {
        ...
        const fakeSession = new ClientSession(null);
        const publish_spy = spyOn<ClientSession>( fakeSession, 'publish');
        publish_spy.and.callFake((request: subscription_service.PublishRequest, callback) => {
            ...
            setTimeout(function () {const fakeResponse = new subscription_service.PublishResponse({subscriptionId: 1});
        ...
        expect( (<any>clientPublishEngine).timeoutHint).toEqual(10000, "expecting timeoutHint to be set to default value =10sec");
...
//        ClientSidePublishEngine.publishRequestCountInPipeline = 0;
    ...
    it('a client should acknowledge sequence numbers received in PublishResponse in next PublishRequest', function () {
        ...
        responses.push(new subscription_service.PublishResponse({
            subscriptionId: 1,
        ...
        if (publishRequest2) {
        expect(publishRequest2.subscriptionAcknowledgements)
            .toEqual([new SubscriptionAcknowledgement({sequenceNumber: 36, subscriptionId: 1})]);
        }
            ...
            if (publishRequest3) {
            expect(publishRequest3.subscriptionAcknowledgements)
                .toEqual([new SubscriptionAcknowledgement({sequenceNumber: 78, subscriptionId: 44})]);
            }
    ...
    it('#390 should not send publish request if channel is not properly opened', function (done) {
        ...
        // start a first new subscription
        clientPublishEngine.registerSubscription(makeSubscription(
            fakeSession, 1, 20000));
