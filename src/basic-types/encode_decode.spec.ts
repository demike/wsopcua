 ...
 */
function test_encode_decode(obj, encode_func, decode_func, expectedLength: number, verify_buffer_func?: (buf: ArrayBuffer) => void) {
...
describe('encoding and decoding arrays', function() {
    ...
    it('should encode and decode an array of ByteString', function() {
        ...
        let data = [ new TextEncoder().encode('ABCD'), null, new ArrayBuffer(0), []];
        data = data.map(ec.coerceByteString);

        test_encode_decode(data, encode_array_string, decode_array_string, 24 /* 4B len=4 + (4B len=4 + 4B Data) + (4B len=-1 ) + (4B len=0) + (4B len=0)  */);
    });
});
...
describe('check isValid and random for various types', function() {
    ...
    types.forEach(function(type) {
        it('should have a random and isValid method for type ' + type, function() {
            ...
            expect(ec['encode' + type]).toBeDefined();
            expect(ec['decode' + type]).toBeDefined();
