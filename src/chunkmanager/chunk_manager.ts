...
export function verify_message_chunk(message_chunk: DataView | ArrayBuffer) {
  ...
  if (message_chunk.byteLength !== header.length) {
    throw new Error(
      ' chunk length = ' + message_chunk.byteLength + ' message  length ' + header.length
    );
  }
}
...
export type WriteHeaderFunc = (
  chunk: DataStream | DataView,
  isLast: boolean,
...
export class ChunkManager extends EventEmitter<ChunkManagerEvents> {
  ...
  constructor(options: IChunkManagerOptions) {
    ...
    this.sequenceHeaderSize =
      options.sequenceHeaderSize === undefined ? 8 : options.sequenceHeaderSize;
    ...
    if (this.cipherBlockSize === 0) {
      ...
      // unencrypted block
      this.maxBodySize =
        this.chunkSize - this.headerSize - this.signatureLength - this.sequenceHeaderSize;
    } else {
      assert(this.plainBlockSize !== 0);
      ...
      this.maxBodySize =
        this.plainBlockSize *
          Math.floor(
            (this.chunkSize - this.headerSize - this.signatureLength - 1) / this.cipherBlockSize
          ) -
        this.sequenceHeaderSize;
