...
fs.readFile(path.join(datafolder, '/StatusCodes.csv'), "utf8", (err, data) => {
    if (err) {
        console.log(err);
    }
    else {
...
function parseStatusCodeXML() {
    ...
    code_list.forEach(function (obj) {
        var s = sep + " " + obj.name + ": { name: '" + obj.name + "' , value: 0x" + obj.value.toString(16) + " ,description: \"" + obj.description + "\"}\n";
        outFile.write(s);
    });
    outFile.write("};\n");
}
//# sourceMappingURL=generate_status_code.js.map