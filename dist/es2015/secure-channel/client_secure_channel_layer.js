...
export function dump_transaction_statistics(stats) {
    function w(str) {
...
export class ClientSecureChannelLayer extends EventEmitter {
    ...
    _open_secure_channel_request(is_initial, callback) {
        assert(this._securityMode !== MessageSecurityMode.Invalid, "invalid security mode");
        // from the specs:
    ...
    _on_connection(transport, callback, err) {
        /* jshint validthis: true */
        /* istanbul ignore next */
        if (err) {
    ...
    create(endpoint_url, callback) {
        ...
        // Handle reconnection
        // --------------------------------------------------------------------------
        var _establish_connection = (transport, endpoint_url, callback) => {
            var last_err = null;
        ...
        switch (this._securityMode) {
            ...
            case MessageSecurityMode.SignAndEncrypt:
                ...
                securityHeader = new AsymmetricAlgorithmSecurityHeader({
                    ...
                    senderCertificate: this.getCertificateChain(),
