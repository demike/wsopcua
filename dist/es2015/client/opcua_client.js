...
export class OPCUAClient extends OPCUAClientBase {
    constructor(options) {
        super(options);
        this._closeSession = function (session, deleteSubscriptions, callback) {
    ...
    __createSession_step2(session, callback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let applicationUri = yield this._getApplicationUri();
    ...
    reactivateSession(session, callback) {
        ...
        let old_client = session.client;
        debugLog("OPCUAClient#reactivateSession");
        this._activateSession(session, (err) => {
            if (!err) {
                if (old_client !== this) {
                    // remove session from old client:
                    old_client._removeSession(session); //cast to any as access of protected member to other instance should be possible
                    assert(old_client._sessions.indexOf(session) < 0 /*!_.contains( (<any>old_client)._sessions, session*/);
...
function findUserTokenPolicy(endpoint_description, userTokenType) {
    ...
    var r = endpoint_description.userIdentityTokens.filter(function (userIdentity) {
...
function createAnonymousIdentityToken(session) {
    ...
    let userTokenPolicy = findUserTokenPolicy(endpoint_desc, UserIdentityTokenType.ANONYMOUS);
    if (!userTokenPolicy) {
