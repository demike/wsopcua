...
var crypto = window.crypto || window.msCrypto; // for IE 11//require("crypto");
export function makeApplicationUrn(hostname, suffix) {
    ...
    if (hostname_hash.length + 7 + suffix.length >= 64) {
        ...
        p = crypto.subtle.digest('md5', str2arraybuffer(hostname)).then((value) => crypto.subtle.digest('hex', value)).then((value) => arraybuffer2str(value).substr(0, 16));
    }
    else {
